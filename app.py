...
# step 5: divide data info training & testing part 
...
#show data sample
st.write(df.head())
#create side for user input form
st.sidebar.title("Fill Customer Details")
st.sidebar.image(img_url)

#To get user input
all_ans = []
for index, col_name in enumerate(X.colums):
  min_v = X[col_name].min()
  max_v = X[col_name].max()
  if col_name != "Previous_Insureance":
...
final_ans = model.predict([all_ans])[]
if final_ans == 0:
  st.info("❌Customer will not buy Insurance❌")
else:
